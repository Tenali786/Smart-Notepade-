...
import os
from tkinter.constants import BOTH, END, LEFT, UNDERLINE, W, X
...
def Open_New(event=None):
    ...
    try:
        ...
        if text_changed:
            ...
            mbox = messagebox.askyesno(title="Save File",message="Do you want to save file")
            if mbox is True:
                ...
                if url:
                    ...
                    with open(url,'w',encoding='utf-8') as fw:
                        fw.write(content)
                    print('already')
                    url = filedialog.askopenfilename(title='Select File',filetypes=(('Text','.txt'),('All Files','*.*')))
                    ...
                    print('Save New')
                    url  = filedialog.asksaveasfilename(title='Save File Name',defaultextension='.txt',filetypes=(('Text','.txt'),('All Files','*.*')))
                    with open(url,'w',encoding='utf-8') as fw:
                        fw.write(content2)
...
def Save_File(event=None):
    ...
    try:
        if url:
            ...
            url = filedialog.asksaveasfilename(title='Save File Name',defaultextension='.txt',filetypes=(('Text','*.txt'),('All File','*.*')))
            contents = str(text_editor.get(1.0 ,END))
            with open(url,'w',encoding="utf-8") as fw:
                fw.write(contents)
            main_appication.title(os.path.basename(url))
    except:
...
def Exit_File(event=None):
    ...
    try:
        if text_changed:
            mbox = messagebox.askyesnocancel(title="Save File",message="Do You Want To Save File")
            if mbox is True:
...
edit.add_command(label='Copy',compound=LEFT,image=copy_icon,accelerator='ctrl+C',command=lambda:text_editor.event_generate("<Control c>"))
edit.add_command(label='Paste',compound=LEFT,image=paste_icon,accelerator='ctrl+V',command=lambda:text_editor.event_generate("<Control v>"))
edit.add_command(label='Cut',compound=LEFT,image=cut_icon,accelerator='ctrl+X',command=lambda:text_editor.event_generate("<Control x>"))
edit.add_command(label='Clear all',compound=LEFT,image=clear_icon,accelerator='ctrl+Z',command=lambda:text_editor.delete(1.0 ,END))
